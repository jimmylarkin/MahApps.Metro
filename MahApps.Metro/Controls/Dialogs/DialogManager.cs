...
namespace MahApps.Metro.Controls.Dialogs
{
    public static class DialogManager
    {
        ...
        /// <summary>
        /// Creates a custom input dialog inside of the current window.
        /// </summary>
        /// <param name="window">The window.</param>
        /// <param name="dialog">The dialog instance itself.</param>
        /// <param name="settings">Optional settings that override the global metro dialog settings.</param>
        /// <returns>
        /// The input that was captured by the dialog or null (Nothing in Visual Basic) if the user cancelled the operation.
        /// </returns>
        public static Task<T> ShowInputAsync<T>(this MetroWindow window, BaseMetroDialog dialog, MetroDialogSettings settings = null)
        {
            window.Dispatcher.VerifyAccess();
            ...
            return HandleOverlayOnShow(settings, window).ContinueWith(z =>
            {
                return (Task<T>)window.Dispatcher.Invoke(new Func<Task<T>>(() =>
                {
                    if (settings == null)
                        settings = window.MetroDialogOptions;
                    ...
                    return dialog.WaitForLoadAsync().ContinueWith(x =>
                    {
                        ...
                        return ((InputDialog<T>)dialog).WaitForButtonPressAsync().ContinueWith(y =>
                        {
                            //once a button as been clicked, begin removing the dialog.
                            ...
                            Task closingTask = (Task)window.Dispatcher.Invoke(new Func<Task>(() => dialog._WaitForCloseAsync()));
                            return closingTask.ContinueWith<Task<T>>(a =>
                            {
                                return ((Task)window.Dispatcher.Invoke(new Func<Task>(() =>
                                {
                                    window.SizeChanged -= sizeHandler;
